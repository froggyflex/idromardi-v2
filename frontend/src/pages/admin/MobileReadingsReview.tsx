      ...
      <div className="grid gap-4 xl:grid-cols-2">
        {rows.map((row) => {
          ...
          return <article key={row.id} className="rounded-2xl border bg-white p-5 shadow-sm">
            <div className="flex justify-between gap-3"><div><div className="font-bold">{row.condominio_nome}</div>
              <div className="text-sm text-slate-600">#{row.id_user} · Interno {row.Interno || "-"} · {row.Nome} {row.Cognome}</div></div>
